...
public class Rev {
    @InjectLinks({
            ...
            @InjectLink(resource = UserResource.class, method = "getRev", style = InjectLink.Style.ABSOLUTE, rel = "user-profile", title = "User profile", bindings = @Binding(name = "id", value = "${instance.userid}")),
            @InjectLink(resource = RevResource.class, method = "getRev", style = InjectLink.Style.ABSOLUTE, rel = "next", title = "Newer rev", bindings = {@Binding(name = "timestamp", value = "${instance.creationTimestamp}"), @Binding(name = "before", value = "false")}),
            @InjectLink(resource = RevResource.class, method = "getRev", style = InjectLink.Style.ABSOLUTE, rel = "previous", title = "Older rev", bindings = {@Binding(name = "timestamp", value = "${instance.creationTimestamp}"), @Binding(name = "before", value = "true")}),
    })
