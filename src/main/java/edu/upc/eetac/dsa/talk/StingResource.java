...
import edu.upc.eetac.dsa.talk.dao.StingDAO;
import edu.upc.eetac.dsa.talk.dao.StingDAOImpl;
...
public class StingResource {
    ...
    public Response createSting(@FormParam("subject") String subject, @FormParam("content") String content, @Context UriInfo uriInfo) throws URISyntaxException {
        ...
        return Response.created(uri).type(TalkMediaType.TALK_STING).entity(sting).build();
    }

    @GET
    @Produces(TalkMediaType.TALK_STING_COLLECTION)
    ...
    @Path("/{id}")
    @GET
    @Produces(TalkMediaType.TALK_STING)
    ...
    @Path("/{id}")
    @PUT
    @Consumes(TalkMediaType.TALK_STING)
    @Produces(TalkMediaType.TALK_STING)
